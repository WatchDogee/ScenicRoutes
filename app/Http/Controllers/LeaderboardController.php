...
class LeaderboardController extends Controller
{
    ...
    public function topRatedRoads(Request $request)
    {
        ...
        $roads = Cache::remember($cacheKey, 300, function () use ($limit) {
    ...
    {
        $limit = $request->input('limit', 10);
        $cacheKey = "leaderboard_most_reviewed_roads_{$limit}";
    ...
    public function popularRoadsByCountry(Request $request)
    {
        try {
            ...
            $roads = $query->orderBy('reviews_avg_rating', 'desc')
                ->orderBy('reviews_count', 'desc')
                ->take($limit * 10) 
                ->get();
    ...
    public function topRatedCollections(Request $request)
    {
        try {
            $limit = $request->input('limit', 10);
            $cacheKey = "leaderboard_top_rated_collections_{$limit}";

            $collections = Cache::remember($cacheKey, 600, function () use ($limit) {
                // Get collections with reviews and calculate average rating
                return Collection::where('is_public', true)
                    ->with(['user:id,name,username,profile_picture', 'tags', 'reviews:collection_id,rating', 'roads' => function($query) {
                        $query->select('saved_roads.id', 'road_name', 'road_coordinates', 'length', 'average_rating', 'country', 'region')
                            ->limit(3); 
                    }])
                    ->withCount('reviews')
                    ...
                    ->map(function($collection) {
                        // Calculate average rating from reviews if not set
                        if (!$collection->average_rating || $collection->average_rating == 0) {
                            $reviews = $collection->reviews;
                            if ($reviews && $reviews->count() > 0) {
                                $avgRating = $reviews->avg('rating');
                                $collection->average_rating = $avgRating ? round($avgRating, 2) : 0;
                            } else {
                    ...
                    ->filter(function($collection) {
                        // Only include collections with ratings > 0
                        return $collection->average_rating > 0;
                    })
            ...
            Log::error('Error in topRatedCollections: ' . $e->getMessage() . ' | Trace: ' . $e->getTraceAsString());
