...
export default function Login({ status }) {
    ...
    const submit = async (e) => {
        ...
        setResendError('');
        try {
            // Post to /login API endpoint using apiClient
            ...
            if (error.response?.status === 403 && error.response?.data?.verification_needed) {
                // Show inline resend prompt on the login screen
                console.log('Showing unverified prompt with email:', error.response?.data?.email);
                setUnverified(true);
                setUnverifiedEmail(error.response?.data?.email || data.login || '');
            } else {
                // Handle other errors
    ...
    const resendVerificationEmail = async () => {
        const emailToUse = unverifiedEmail || data.login;
