...
export default function CollectionRatingModal({ isOpen, onClose, onSubmit, collection, auth, initialRating = 0, initialComment = '' }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Call the parent callback with the updated collection
            if (response.data.collection) {
                onSubmit(rating, comment, response.data.collection);
            } else {
                // Still call with updated reviews even if collection data is incomplete
                onSubmit(rating, comment, { ...collection, reviews });
            }
        } catch (error) {
