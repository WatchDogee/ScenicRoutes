...
export default function SaveToCollectionModal({
    ...
    const handleAddToCollection = async () => {
        ...
        try {
            ...
            console.error('Error adding road to collection:', error);
    ...
    const handleCreateNew = () => {
        // Store the road data locally before closing the modal
        const roadData = { ...roadToAdd };

        // First close this modal completely and reset state
        onClose();

        // Use a longer timeout to ensure the modal is fully closed and removed from DOM
        setTimeout(() => {
            // Check if onCreateNew callback exists and the component is still mounted
            if (onCreateNew) {
                console.log('SaveToCollectionModal: Calling onCreateNew with road:', roadData.id);
                onCreateNew(roadData);
            }
        }, 300); // Longer delay to ensure proper modal transition and DOM cleanup
    };
