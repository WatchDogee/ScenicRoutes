...
export default function Leaderboard({ onViewRoad, onViewUser, onViewRoadDetails }) {
    ...
    useEffect(() => {
        const fetchPopularRoadsByCountry = async () => {
            try {
                ...
                console.error('Error fetching popular roads by country:', error);
    ...
    // Function to render popular roads by country
    const renderPopularRoadsByCountry = () => {
        // Using the state variables defined at the top level of the component
        ...
        if (errorCountryRoads) {
            return (
                <div className="text-center py-8 text-red-500">
                    ...
                    <button
                        onClick={() => {
                            // Trigger a re-fetch by toggling the activeTab
                            setActiveTab('temp');
    ...
    // Set default active tab when category changes
    useEffect(() => {
        if (categoryType === 'roads') setActiveTab('top-rated');
        else if (categoryType === 'collections') setActiveTab('featured-collections');
        else if (categoryType === 'users') setActiveTab('most-active'); // Changed from 'top-curators' to 'most-active'
    }, [categoryType]);
