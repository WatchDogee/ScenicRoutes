...
export default function EnhancedCollections({
    ...
    const fetchCollections = async () => {
        ...
        try {
            if (isAuthenticated && user) {
                ...
                const createdData = createdResponse.data.data || createdResponse.data.collections || createdResponse.data || [];
                const filtered = Array.isArray(createdData) ? createdData.filter(
                    c => c.user && c.user.id === user.id
                ) : [];
                setMyCreatedCollections(filtered);
                try {
                    const savedResponse = await axios.get('/api/saved-collections');
                    const savedData = savedResponse.data.data || savedResponse.data.collections || savedResponse.data || [];
                    setMySavedCollections(Array.isArray(savedData) ? savedData.filter(
                        c => c.user && c.user.id !== user.id
                    ) : []);
                } catch (savedErr) {
            ...
            setCommunityCollections(communityResponse.data.data || communityResponse.data.collections || communityResponse.data || []);
            setError(null);
        } catch (err) {
    ...
    const handleSaveCollection = async (collection) => {
        if (!isAuthenticated) {
            showToast('Please log in to save collections', 'warning', 3000);
            return;
        }
        try {
            const response = await axios.post(`/api/collections/${collection.id}/save`);
            showToast('Collection saved successfully!', 'success', 3000);
            fetchCollections();
        } catch (err) {
    ...
    const handleRemoveSavedCollection = async (collection) => {
        if (!isAuthenticated) return;
        if (!window.confirm('Remove this collection from your saved list?')) return;
        try {
            await axios.delete(`/api/saved-collections/${collection.id}`);
            fetchCollections();
        } catch (err) {
            showToast('Failed to remove collection from your saved list.', 'error', 4000);
        }
    };
    ...
    const renderCollectionCard = (collection) => {
        const isCreatedByMe = collection.user?.id === authState.user?.id;
        const isSavedFromOthers = !isCreatedByMe && mySavedCollections.some(c => c.id === collection.id);
        return (
            <div
                key={collection.id}
                className="border rounded-lg p-4 hover:bg-gray-50 shadow-sm transition-all hover:shadow-md"
            >
                <div className="flex items-center mb-2">
                    ...
                    <div className="ml-3 flex-1">
                        ...
                        <div className="flex items-center text-sm text-gray-600">
                            ...
                            <span className={`flex items-center cursor-pointer hover:text-blue-600 ${isCreatedByMe ? 'font-semibold text-blue-600' : ''}`}
                                onClick={(e) => {
                                    e.stopPropagation();
                                    if (collection.user) {
                ...
                <div className="mt-3 flex justify-end">
                    <button
                        onClick={(e) => {
                            e.stopPropagation();
                            handleViewCollectionDetails(collection);
                        }}
                ...
                {isSavedFromOthers && (
                    <div className="mt-3 flex justify-end">
                        <button
                            onClick={(e) => {
                                e.stopPropagation();
                                handleRemoveSavedCollection(collection);
                            }}
                            ...
                            <>
                                {!isAuthenticated ? (
                                    <div className="text-center py-8 bg-gray-50 rounded border">
                                        ...
                                        <p className="text-gray-600 mb-2">Please log in to view your collections</p>
                                        <p className="text-sm text-gray-500">You need to be logged in to see collections you've created or saved.</p>
                                    </div>
                                ) : (
                                    <>
                                        ...
                                        {/* Empty States */}
                                        {myCollectionsViewMode === 'all' && myCreatedCollections.length === 0 && mySavedCollections.length === 0 && (
