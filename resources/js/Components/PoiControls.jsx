...
export default function PoiControls({
    ...
    const handleFetchPois = () => {
        ...
        if (!auth?.user) {
            alert('Please log in to search for Points of Interest. POI search is available for Premium and Pro users.');
            return;
        }

        // Check tier
        const tier = auth.user.subscription?.plan || 'free';
        if (tier === 'free') {
            alert('Points of Interest search is available for Premium and Pro users. Please upgrade to access this feature.');
            return;
        }
