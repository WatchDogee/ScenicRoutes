...
const SaveRoadModal = ({
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const payload = {
                road_name: roadName,
                route_type: 'road', // Mark as road (not route)
            ...
            setError(error.response?.data?.message || 'Failed to save road.');
        } finally {
            setIsSaving(false);
        }
    };
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className="mb-6 bg-gray-100 p-4 rounded">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                {roadData.elevation_gain !== null && roadData.elevation_gain !== undefined && formatElevation(roadData.elevation_gain) !== 'N/A' && (
                                    <div>
                                        <p className="text-sm text-gray-600">Elevation Gain:</p>
                                        <p className="font-semibold">{formatElevation(roadData.elevation_gain)}</p>
                                    </div>
                                )}
                                {roadData.elevation_loss !== null && roadData.elevation_loss !== undefined && formatElevation(roadData.elevation_loss) !== 'N/A' && (
                                    <div>
                                        <p className="text-sm text-gray-600">Elevation Loss:</p>
                                        <p className="font-semibold">{formatElevation(roadData.elevation_loss)}</p>
                                    </div>
                                )}
                                {roadData.max_elevation !== null && roadData.max_elevation !== undefined && formatElevation(roadData.max_elevation) !== 'N/A' && (
                                    <div>
