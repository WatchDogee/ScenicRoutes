// Trigger redeploy: no-op change for GitHub Actions
import React, { useState, useEffect } from 'react';
import axios from 'axios';
import { FaPlus, FaTrophy, FaUsers, FaUserFriends, FaRoad, FaFolder, FaTimes, FaSearch, FaTag, FaMapMarkerAlt, FaFilter, FaChevronDown, FaSpinner, FaUser } from 'react-icons/fa';
...
export default function SocialModal({ isOpen, onClose, onViewRoad, onViewRoadDetails, selectedCollectionId: initialCollectionId, roadToAdd, activeTab: initialActiveTab, setActiveTab: setParentActiveTab }) {
    ...
    useEffect(() => {
        ...
        const handleCollectionUpdated = (event) => {
            setCollectionRefreshKey(prev => prev + 1);
            refreshCollections();
        };
        
        window.addEventListener('collectionUpdated', handleCollectionUpdated);
        ...
        return () => {
            ...
            window.removeEventListener('collectionUpdated', handleCollectionUpdated);
        };
    }, [isOpen]);
    ...
    const fetchCountries = async () => {
        try {
            ...
            // Removed debug endpoint call - endpoint doesn't exist
    ...
    useEffect(() => {
        if (isOpen && roadToAdd) {
            ...
            if (isNewRoadAddition) {                setActiveTab('collections');
                if (setParentActiveTab) {
                    setParentActiveTab('collections');
                }
    ...
    const fetchCollections = async () => {
        try {
            ...
            try {
                ...
                console.error('Failed to fetch collections:', apiError);
                ...
                if (!searchLocation) {
                    ...
                    try {
                        ...
                        if (response.data && response.data.roads && Array.isArray(response.data.roads)) {
                            ...
                            setSearchError('Received invalid data.');
                        }
                    } catch (error) {
                        if (error.response) {
                            if (error.response.status === 404) {
                                setSearchError('The search endpoint was not found.');
                            ...
                            setSearchError('No response received from server.');
                        } else {
    ...
    const searchCollections = async () => {
        ...
        try {
            ...
            if (searchQuery && searchQuery.startsWith('@')) {
                searchParams.username = searchQuery.substring(1);
            }
    ...
    const handleSearch = async (e) => {
        ...
        // Don't search if search query is empty for users
        ...
        if (searchType === 'roads') {
            ...
            try {
                ...
                setUserResults(Array.isArray(response.data) ? response.data : (response.data.data || response.data.users || []));
            } catch (error) {
                setUserResults([]);
            ...
            <div className="space-y-6">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="flex space-x-2">
                        ...
                        <button
                            className={'px-3 py-1 rounded-md ' + (searchType === 'users' ? 'bg-blue-500 text-white' : 'bg-gray-200')}
                                ...
                                <div className="space-y-4">
                                    {userResults.length === 0 ? (
                                        <div className="text-center py-8 bg-gray-50 rounded border">
                                            ...
                                            <p className="text-sm text-gray-500 mt-1">Try searching for a different username or name.</p>
                                        </div>
                                    ) : (
                                        <>
                                            <p className="text-sm text-gray-600">Found {userResults.length} users</p>
                                            <div className="space-y-4">
                                                {userResults.map(user => (
                                                    <div key={user.id} className="border rounded-lg p-4 flex items-center hover:bg-gray-50 cursor-pointer" onClick={() => handleViewUser(user)}>
    ...
    return (
        <>
            ...
            <CollectionDetailsModal
                key={`collection-${selectedCollectionId}-${collectionRefreshKey}`}
