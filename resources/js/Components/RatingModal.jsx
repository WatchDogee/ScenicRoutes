...
import RoadComments from './RoadComments';
...
export default function RatingModal({ isOpen, onClose, onSubmit, road, auth, initialRating = 0, initialComment = '' }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (roadPhotos.length > 0) {
                const formData = new FormData();
                formData.append('rating', rating);
                if (comment) formData.append('comment', comment);
                roadPhotos.forEach((photo, index) => {
    ...
    return (
        <Portal rootId="rating-modal-root">
            ...
            >
                ...
                {/* Road Comments Section */}
                <RoadComments roadId={road.id} auth={auth} />
                </div>
            </div>
        </Portal>
    );
